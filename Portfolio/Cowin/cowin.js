...
xhr.onreadystatechange = () => {
    if (xhr.status == 200 && xhr.readyState == 4) {
        ...
        state_select.addEventListener('change', (e) => {
            ...
            const district_url = `https://cdn-api.co-vin.in/a/pi/v2/admin/location/districts${state_id}`
                        ...
                        xhr.onreadystatechange = () => {
                            if (xhr.readyState == 4 && xhr.status == 200) {
                                ...
                                for (let i = o; i < test_len; i++) {
                                    output += `<tr> ${data.sessions[i].name} <br>${data.sessions[i].address}</tr><hr>`
                                ...
                                typeofvaccine.innerHTML = output1
                            }
                        }
                            xhr.send()
                    })
